from tkinter import *
import time
from StoppUhr import StoppUhr
import math
root = Tk()
stoppUhr = StoppUhr()
#Der Modus in dem sich das Programm befindet. Uhr = Uhr, Stoppuhr = Stoppuhr, Timer = Timer
mode = "Uhr"
#DropDownMenüSize
dropDownSize = 100
...
sizeY = offset_Border*2+offsetBetweenLines*4+sizeLine*2 + 3*lineWidth + dropDownSize
...
def draw_points():
    for x in range(2):
        ...
        for y in range(2):
            ...
            if y == 1:
                reverse = -1
            offsetY = offset_Border+dropDownSize+offsetBetweenLines+sizeLine+offsetBetweenLines+ reverse*(offset_PointsFromMiddle+(size_Points*y))
...
def draw_Vertical_Line(numberPos,id,deleteLine):
    ...
    #Die ErsteY:
    offsetY = offset_Border + dropDownSize + offsetBetweenLines + lower*(sizeLine+2*offsetBetweenLines) + (lower+1)*lineWidth 
    #Die ZweiteY:
...
def draw_Horizontal_Line(numberPos,id,deleteLine):
    ...
    offsetY = offset_Border + dropDownSize + (id/3)*(sizeLine) + 2*(id/3)*offsetBetweenLines + (id/3)*lineWidth + (lineWidth/2)
    if deleteLine == 1:
...
def draw_BC():
    for x in range(6):
        draw_Number(x,8,0)
draw_BC()
def draw_Time(timePar):
    numbersInTime = timePar.split(':')
...
def ui_Update(choice):
    global mode
    choice = var.get()
    mode = choice
    
w = OptionMenu(root,var,*parameters,command = ui_Update)
w.config(width = 50)
w.place(x=0,y=0)
